...
public class CategoryWiseActivity extends AppCompatActivity {
    ...
    private void showSimpleFilterDialog() {
        ...
        // Hardwired defaults at top (in required order)
        List<String> defaults = new ArrayList<>();
        Collections.addAll(defaults, "Groceries", "Rent", "Utilities", "Transport", "Other");

        // Custom categories sorted alphabetically
        List<String> customs = new ArrayList<>();
        for (Expense e : all) {
            if (e.category != null && !defaults.contains(e.category) && !customs.contains(e.category)) {
        ...
        // User-selectable dates (clickable)
        ...
        txtEnd.setPadding(0, dp(4), 0, dp(8));
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Apply", (d, which) -> {
                    ...
                    if (startFriendly.contains("(") || endFriendly.contains("(")) {
                        // No manual range picked — fallback to available range if possible
                        CharSequence available = txtAvailable.getText();
                        if (available.toString().contains("–")) {
                            String[] parts = available.toString().replace("Available: ", "").split("–");
                            if (parts.length == 2) {
